# TASK 4. Car Types
'''
The root of the hierarchy is the abstract base class Car. 
The abstraction is modeled from the perspective of a driver and defines two abstract methods: 
get_remaining_range returns the remaining distance in kilometers (float) 
and drive can be used to drive the car for dist kilometers. Car has two direct subtypes, CombustionCar and ElectricCar. 
Both car types are used differently.

CombustionCars are initiated with the gas_capacity in liters and gas_per_100km that describes the fuel consumption. 
The car can be refueled using fuel, which adds f liters of fuel to the gas tank. The method should raise a Warning, 
if the gas tank gets overfilled. A call to get_gas_tank_status returns the current gas tank capacity c and the maximum capacity c_max as a tuple 
(c, c_max). It must always be that 0 <= c <= c_max. 
To make a CombustionCar usable, the two abstract methods get_remaining_range and drive need to be implemented. 
On use, drive should remove the correct amount of gas from the gas tank and it should raise a Warning 
if the gas tank is fully depleted through driving. Please consider the following example that illustrates a car usage:
